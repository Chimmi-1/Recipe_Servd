...
import { Check, ChefHat, Edit2, Loader2, Package, Plus, Sparkle, Trash2, X } from "lucide-react";
import { useState, useEffect } from "react";
import { Button } from "@/components/ui/button";
import AddToPantryModal from "@/components/AddToPantryModal";
import useFetch from "@/hooks/use-fetch";
import { getPantryItems, deletePantryItems, updatePantryItems } from "@/actions/pantry.actions";
...
const PantryPage = () => {
  ...
  return (
    <div className="min-h-screen bg-stone-50 pt-24 pb-16">
      <div className="container mx-auto max-w-5xl">
        <div className="mb-4">
          <div className="flex items-center justify-between mb-6">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h1 className="text-4xl md:text-5xl font-bold text-stone-900 tracking-tight">My Pantry</h1>
                <p className="text-stone-600 font-light">Manage your ingredients and discover what you can cook</p>
              </div>
            </div>
            ...
            <div className="bg-white py-3 px-4 border-2 border-stone-200 inline-flex items-center gap-3">
              ...
              <div className="text-sm">
                {itemsData.scansLimit === "unlimited" ? (
                  <>
                    <span className="font-bold text-green-600">&infin;</span>
        ...
        {items.length > 0 && (
          <Link href="/pantry/recipes" className="block mb-8">
            <div className="bg-gradient-to-br from-green-600 to-emerald-500 text-white p-6 border-2 border-emerald-700 hover:shadow-xl hover:-translate-y-1 transition-all cursor-pointer group">
              <div className="flex items-center gap-4">
                ...
                <div className="flex-1">
                  ...
                  <p className="text-green-100 text-sm font-light">Get AI-powered recipe suggestions from your {items.length} ingredients</p>
                </div>
                <div className="hidden sm:block">
                  <Badge className="bg-white/20 text-white border-2 border-white/30 font-bold uppercase tracking-wide">{items.length} items</Badge>
                </div>
              </div>
            </div>
          </Link>
        ...
        {!loadingItems && items.length > 0 && (
          <div>
            ...
            <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-4">
              {items.map((item) => {
                const itemKey = item.id || item.documentId;
                return (
                  <div
                    key={itemKey}
                      ...
                      <div className="space-y-3">
                        ...
                        <div className="flex gap-2">
                          <Button
                            ...
                            onClick={saveEdit}
                            disabled={updating}
                            className="flex-1 bg-green-600 hover:bg-green-700 border-2 border-green-700 text-white"
                          >
                      ...
                      <div>
                        <div className="flex items-start justify-between mb-3">
                          ...
                          <div className="flex gap-1">
                            <Button onClick={() => startEdit(item)} variant="ghost">
                              <Edit2 className="w-4 h-4" />
                            </Button>
                            <Button onClick={() => handleDelete(itemKey)} disabled={deleting} variant="ghost">
                              <Trash2 className="w-4 h-4" />
                            </Button>
                          </div>
                        </div>
                        <div className="text-xs text-stone-400">Added {new Date(item.createdAt).toLocaleDateString()}</div>
                      </div>
                    )}
                  </div>
                );
      ...
      <AddToPantryModal isOpen={isModalOpen} onClose={() => setIsModalOpen(false)} onSuccess={handleModalSuccess} />
    </div>
  );
};
